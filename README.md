...
- ✅ **可扩展 API 源架构**：默认源从“固定下标耦合”改为“sourceId + provider”注册模式，更易维护。
- ✅ **运行时自定义 API 源**：在设置页可直接新增 API（源名、URL、关键词参数、结果数组字段、图片字段），无需改代码。
- ✅ **自定义源持久化**：新增的 API 源会保存到用户目录 `~/.emoji-search/custom-sources.json`，重启后自动加载。
- ✅ **交互体验增强**：主界面新增“搜索按钮 + 搜索状态 + 加载进度条”，并优化图片 hover 反馈。
- ✅ **配置持久化**：当前所选抓取源下标持久化至 `~/.emoji-search/app.properties`。
- ✅ **收藏能力**：支持右键收藏图片，新增“收藏源”可直接检索收藏内容；支持右键另存为本地图片。
